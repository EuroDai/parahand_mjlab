...
- ParaHand grasp policies now use a tanh-squashed Gaussian action distribution
  ...
  value.
- ParaHand grasp contact rewards now use a smooth structured force gate that
  requires the thumb and an index-or-middle core contact, rewards simultaneous
  index-and-middle contact, and treats ring and little contacts as weighted
  bonuses instead of substitutes. The score gates contact and position-tracking
  rewards; success remains position-only.
